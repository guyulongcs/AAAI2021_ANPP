...
class Model(object):
  ...
  def build_model(self, cate_list):
    item_emb_w = tf.get_variable(
        "item_emb_w",
        [self.config['item_count']+1, self.config['itemid_embedding_size']])
    ...
    cate_emb_w = tf.get_variable(
        "cate_emb_w",
        [self.config['cate_count']+1, self.config['cateid_embedding_size']])
  ...
  def eval_negN(self, sess, uij, item_count, test_neg_N_item, dict_user_testNegN):
    B = len(uij[0])
    #test
    res = np.zeros((B, 1 + test_neg_N_item))
    #test pos
    ...
    #neg
    # sample neg
    def gen_neg(pos, hist_i):
        neg = pos
        while neg == pos or neg in hist_i:
            neg = random.randint(1, item_count + 1)
        return neg

    neg_test = np.zeros((B, test_neg_N_item))
    for i in range(B):
        u = uij[1][i]
        u_test_negN = dict_user_testNegN[u][0]
        neg_test[i, :] = np.array(u_test_negN)

    #test neg
    for i in range(test_neg_N_item):
