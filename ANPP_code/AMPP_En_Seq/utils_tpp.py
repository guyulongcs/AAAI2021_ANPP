...
def read_data_pkl(eventTrain, eventTest, timeTrain, timeTest, args, time_method="time_norm_by_TimeMinMax",
              pad=True, paddingStyle='post'):
    """Read data from given files and return it as a dictionary."""
    assert len(timeTrain) == len(eventTrain)
    assert len(eventTest) == len(timeTest)

    if(time_method != "time_norm_by_TimeMinMax" and args.dataset in Config.dataset_should_process_time_seq_to_startZero):
        timeTrain = process_time_seq_to_startZero(timeTrain)
        timeTest = process_time_seq_to_startZero(timeTest)
    ...
    def parse_time(y, minTime, maxTime, time_method):
        if(time_method == "time_norm_by_TimeMinMax"):
            res = (y - minTime) / (maxTime - minTime)
            #res = min(res, 1)
        else:
            res = y
        return res
    timeTrainIn = [[parse_time(y, minTime, maxTime, time_method) for y in x[:-1]] for x in timeTrain]
    timeTrainOut = [[parse_time(y, minTime, maxTime, time_method) for y in x[1:]] for x in timeTrain]
    ...
    if(time_method == "time_bucket"):
        pass


    #get time diff
    train_time_in_seq_delta_prev = get_time_delta_prev(train_time_in_seq)
    test_time_in_seq_delta_prev = get_time_delta_prev(test_time_in_seq)
    ...
    dict_res_item = {
        ...
        "train_time_in_seq_delta_prev": train_time_in_seq_delta_prev[:train_cnt,:],
        ...
        "eval_time_in_seq_delta_prev": train_time_in_seq_delta_prev[train_cnt:,:],
        ...
        "test_time_in_seq_delta_prev": test_time_in_seq_delta_prev,
...
def MAE_norm(time_preds, time_true, events_out, time_scale=1., time_parse_log=False):
    """Calculates the MAE between the provided and the given time, ignoring the inf
    and nans. Returns both the MAE and the number of items considered."""
    ...
    if(not time_parse_log):
        mae = np.mean(np.abs(time_preds - clipped_time_true)[is_finite])
        mae *= time_scale
    else:
        mae = np.mean(np.abs(np.exp(time_preds * time_scale) - np.exp(clipped_time_true * time_scale))[is_finite])

    return mae, np.sum(is_finite)
