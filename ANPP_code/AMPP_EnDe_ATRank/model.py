...
class Model(object):
  def __init__(self, config, dict_item_cate):
    self.config = config

    self.dict_item_cate = dict_item_cate
  ...
  def init_placeholders(self):
    ...
    self.cate = tf.placeholder(tf.int32, [None,])

    # [B, T] user's history item id
    self.hist_i = tf.placeholder(tf.int32, [None, None])

    self.hist_cate = tf.placeholder(tf.int32, [None, None])
  ...
  def build_model(self):
    ...
    item_emb_w = tf.concat((tf.zeros(shape=[1, self.config['itemid_embedding_size']]),
                            item_emb_w[:, :]), 0)
    ...
    cate_b = tf.get_variable(
        "cate_b",
        [self.config['cate_count']+1,],
        initializer=tf.constant_initializer(0.0))

    cate_emb_w = tf.get_variable(
        "cate_emb_w",
        [self.config['cate_count'], self.config['cateid_embedding_size']])
    cate_emb_w = tf.concat((tf.zeros(shape=[1, self.config['cateid_embedding_size']]),
                            cate_emb_w[:, :]), 0)


    c_emb = tf.nn.embedding_lookup(cate_emb_w, self.cate)
    c_b = tf.gather(cate_b, self.cate)

    i_emb = tf.concat([
        tf.nn.embedding_lookup(item_emb_w, self.i),
        tf.nn.embedding_lookup(cate_emb_w, self.cate),
        ], 1)
    i_b = tf.gather(item_b, self.i)

    h_emb = tf.concat([
        tf.nn.embedding_lookup(item_emb_w, self.hist_i),
        tf.nn.embedding_lookup(cate_emb_w, self.hist_cate),
        ], 2)
    ...
    if(self.config["train_type"] == "item"):
    ...
    if (self.config["train_type"] == "category"):
        c_emb = tf.layers.dense(c_emb, self.config['hidden_units'])
        u_emb, self.att, self.stt = attention_net(
            h_emb,
            self.sl,
            c_emb,
            ...
            self.is_training,
            False)
        self.logits = c_b + tf.reduce_sum(tf.multiply(u_emb, c_emb), 1)
  ...
  def train(self, sess, uij, l, add_summary=False):
    (u, i, cate, y, hist_i, hist_cate, hist_t, sl) = uij
    input_feed = {
        self.u: uij[0],
        self.i: uij[1],
        self.cate: uij[2],
  ...
  def eval_negN(self, sess, uij, item_count, test_neg_N_item, test_neg_N_cate):
    (u, item, cate, hist_i, hist_cate, hist_t, sl, negN_item, negN_cate) = uij
    B = len(u)

    test_neg_N = 0
    if(self.config["train_type"] == "item"):
        test_neg_N = test_neg_N_item
        neg_test = negN_item
    if (self.config["train_type"] == "category"):
        test_neg_N = test_neg_N_cate
        neg_test = negN_cate
        #print("neg_test:", negN_cate)

    #test
    res = np.zeros((B, 1 + test_neg_N))

    #test pos
    res_pos = sess.run(self.eval_logits, feed_dict={
        self.u: u,
        self.i: item,
        self.cate: cate,
        self.hist_i: hist_i,
    ...
    #test neg
    for i in range(test_neg_N):
        ivalue = item
        cvalue = cate
        if (self.config["train_type"] == "item"):
            ivalue = neg_test[:, i]
            cvalue = [self.dict_item_cate[tmp] for tmp in ivalue]
        if (self.config["train_type"] == "category"):
            cvalue = neg_test[:, i]
            ivalue = [0 for tmp in cvalue]

        res_i = sess.run(self.eval_logits, feed_dict={
            self.u: u,
            self.i: ivalue,
            self.cate: cvalue,
            self.hist_i: hist_i,
    ...
    #print("res", res)
    N, NDCG_K, Hit_K = Metrics.NDCG_HIT(res, Config.metrics_K)
