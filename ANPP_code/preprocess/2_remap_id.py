...
arr = sys.argv
assert (len(arr) == 1+6)
folder_src, pkl_review, pkl_meta, folder_dst, file_join, pkl_remap = arr[1], arr[2], arr[3], arr[4], arr[5], arr[6]

#files
file_pkl_review = os.path.join(folder_src, pkl_review)
file_pkl_meta = os.path.join(folder_src, pkl_meta)
file_join = os.path.join(folder_dst, file_join)
file_pkl_remap = os.path.join(folder_dst, pkl_remap)

file_desc = os.path.join(folder_src, Config.file_dataset_desc)

print "\nfile_pkl_review:%s" % file_pkl_review
...
with open(file_desc, 'w') as f:
    ...
    for user_id, h in join_df.groupby('user'):
        h_t = h['timestamp'].tolist()
        for i in range(1, len(h_t)):
            gap = (h_t[i] - h_t[i - 1]) / (24 * 3600)
            list_gap.append(gap)
