...
from utils_tpp import create_dir, variable_summaries, MAE, ACC, MAE_last, Rank_last
...
class IntensityRNN:
    ...
    def __init__(self, sess, num_categories, square_std, args, batch_size=def_opts["batch_size"],
                 learning_rate=def_opts["learning_rate"], momentum=def_opts["momentum"], l2_penalty=def_opts["l2_penalty"], 
                 embed_size=def_opts["embed_size"],
                 float_type=def_opts["float_type"], bptt=def_opts["bptt"], seed=def_opts["seed"], 
                 scope=def_opts["scope"], save_dir=def_opts["save_dir"],
                 decay_steps=def_opts["decay_steps"], decay_rate=def_opts["decay_rate"],
                 device_gpu=def_opts["device_gpu"], device_cpu=def_opts["device_cpu"], 
                 summary_dir=def_opts["summary_dir"], cpu_only=def_opts["cpu_only"],
                 Wt=def_opts["Wt"], Wem=def_opts["Wem"], Wh=def_opts["Wh"], 
                 bh=def_opts["bh"], wt=def_opts["wt"], Wy=def_opts["Wy"], 
                 Vy=def_opts["Vy"], Vt=def_opts["Vt"], bk=def_opts["bk"], bt=def_opts["bt"]):
                    ...
                    for i in range(self.BPTT):
                        ...
                        with tf.name_scope('loss_calc'):
                            ...
                            if(self.loss_time_method == "intensity"):
                                ...
                                time_loss_gaussian = tf.multiply(tf.divide(1, tf.sqrt(2*tf.constant(np.pi)*self.sigma_square)), tf.exp(tf.divide(tf.pow(tf.subtract(time_preds, delta_t_next), 2), -2 * self.sigma_square)))
                            ...
                            step_LL = self.weight_loss_event * mark_LL + self.weight_loss_time * time_LL
    ...
    def train(self, training_data, num_epochs=1,
              ...
              with_summaries=False, with_evals=False, flag_train_mini=False):
        ...
        for epoch in range(self.last_epoch, self.last_epoch + num_epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                if batch_idx % 10 == 0:
                    print("\tLoss during Epoch {}, batch {} / {} last BPTT batch loss = {:.5f}, Epoch Total loss: {:.5f}, "
                      "lr = {:.5f}, batch_num_events: {}".format(epoch+1, batch_idx, n_batches, batch_loss, total_loss, self.sess.run(self.learning_rate), batch_num_events))

                if(flag_train_mini and batch_idx > 1000):
                    break
    ...
    def predict(self, event_in_seq, time_in_seq, single_threaded=False, MP_Pool=100):
        ...
        for bptt_idx in range(0, len(event_in_seq[0]) - self.BPTT, self.BPTT):
            ...
            bptt_hidden_states, bptt_events_pred, cur_state, bptt_time_preds = self.sess.run(
                [self.hidden_states, self.event_preds, self.final_state, self.time_preds],
                feed_dict=feed_dict
            )
    ...
    def predict_last(self, event_in_seq, time_in_seq, single_threaded=False):
        ...
        for bptt_idx in [len(event_in_seq[0]) - self.BPTT - 1]:
            bptt_range = range(bptt_idx, (bptt_idx + self.BPTT))
            bptt_event_in = event_in_seq[:, bptt_range]
            bptt_time_in = time_in_seq[:, bptt_range]
        ...
        print("self.loss_time_method:", self.loss_time_method)
        if(self.loss_time_method == "intensity"):
            print("predict time intensity...")
            # TODO: This calculation is completely ignoring the clipping which
            # happens during the inference step.
    ...
    def eval(self, time_preds, time_true, event_preds, event_true, time_scale=1, K=20):
        ...
        print("valid:{}, NDCG:{:.3f}, Hit:{:.3f}, K:{}".format(N, NDCG, Hit, K))
    ...
    def predict_test_last(self, data, single_threaded=False):
        """Make (time, event) predictions on the test data."""
        return self.predict_last(event_in_seq=data['test_event_in_seq'],
                            time_in_seq=data['test_time_in_seq'],
                            single_threaded=single_threaded)
